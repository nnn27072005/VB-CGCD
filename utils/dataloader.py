...
class IncrementalLoader(ABC):
    ...
    def __init__(self, data_dir, pretrained_model_name, base, increment):
        ...
        self.train_images, self.test_images = self._load_images(data_dir, pretrained_model_name)
    ...
    def _load_images(self, path, model_name):
        model_dir = Path(path) / model_name
        train_images_path = model_dir / f"train_images-{model_name}.npy"
        test_images_path = model_dir / f"test_images-{model_name}.npy"

        if train_images_path.exists() and test_images_path.exists():
            return (
                np.load(train_images_path, allow_pickle=True),
